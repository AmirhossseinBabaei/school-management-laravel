...
@section('title', 'مدیریت درس ها | سیستم مدیریت مدرسه')
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                <h3 class="mb-2 gradient-text fw-bold">
                    <i class="fa-solid fa-user-shield me-2"></i>مدیریت درس ها
                </h3>
                <p class="text-muted mb-0">مدیریت درس‌ها </p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('dashboard.lessons.create') }}" class="btn bg-warning btn-pill">
                    <i class="fa-solid fa-plus me-2"></i>افزودن درس جدید
                </a>
            </div>
        </div>
    </div>
                        ...
                        <tr class="animate__animated animate__fadeInUp" style="animation-delay: {{ $key * 0.1 }}s;">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-semibold">{{ $lesson->name ?? 'نامشخص' }}</div>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-semibold">{{ $lesson->study_field->name ?? 'نامشخص' }}</div>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-semibold">{{ $lesson->study_base->name ?? 'نامشخص' }}</div>
                            ...
                            <td>
                                <span
                                    class="text-muted">{{ $lesson->updated_at ? $lesson->updated_at : 'نامشخص' }}</span>
                            </td>
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('dashboard.lessons.show', $lesson->id) }}"
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.delete-btn')) {
                ...
                if (confirm('آیا مطمئن هستید که می‌خواهید این درس را حذف کنید؟')) {
                    const lessonId = e.target.closest('.delete-btn').dataset.id;
                    const url = "{{ route('dashboard.lessons.destroy', ':id') }}".replace(':id', lessonId);
