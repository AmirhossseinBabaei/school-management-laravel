...
@section('title', 'مدیریت  ترم ها | سیستم مدیریت مدرسه')
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                <h3 class="mb-2 gradient-text fw-bold">
                    <i class="fa-solid fa-user-shield me-2"></i>مدیریت ترم ها
                </h3>
                <p class="text-muted mb-0">مدیریت ترم‌های تحصیلی</p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('dashboard.terms.create') }}" class="btn bg-warning btn-pill">
                    <i class="fa-solid fa-plus me-2"></i>افزودن ترم جدید
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="card-header glass-effect border-0">
            <div class="d-flex align-items-center justify-content-between">
                <h5 class="mb-0 gradient-text fw-bold">
                    <i class="fa-solid fa-list me-2"></i>لیست ترم ها
                </h5>
                <div class="d-flex gap-2">
            ...
            <div class="table-responsive">
                <table id="dataTable" class="table table-hover align-middle" style="width:100%">
                    ...
                    <tr>
                        ...
                        <th>
                            <i class="fa-solid fa-user-shield me-2"></i>نام ترم
                        </th>
                        <th>
                            <i class="fa-solid fa-calendar me-2"></i> نام مدرسه
                        </th>
                        <th>
                            <i class="fa-solid fa-chart-bar me-2"></i>سال تحصیلی
                        </th>
                        <th>
                            <i class="fa-solid fa-chart-bar me-2"></i>از تاریخ
                        </th>
                        <th>
                            <i class="fa-solid fa-chart-bar me-2"></i>تا تاریخ
                        </th>
                        <th>
                            <i class="fa-solid fa-chart-bar me-2"></i>تاریخ ایجاد
                        </th>
                        ...
                        <tr class="animate__animated animate__fadeInUp" style="animation-delay: {{ $key * 0.1 }}s;">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-semibold">{{ $term->name ?? 'نامشخص' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="fw-semibold">{{ $term->school->name ?? 'نامشخص' }}</div>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="fw-semibold">{{ $term->study_year ?? 'نامشخص' }}</div>
                                    <small class="text-muted">سال تحصیلی</small>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="fw-semibold">{{ $term->from_date ?? 'نامشخص' }}</div>
                                    <small class="text-muted">از تاریخ</small>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="fw-semibold">{{ $term->to_date ?? 'نامشخص' }}</div>
                                    <small class="text-muted">تا تاریخ</small>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <div class="fw-semibold">{{ $term->created_at ?? 'نامشخص' }}</div>
                                    <small class="text-muted">تاریخ ایجاد</small>
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('dashboard.terms.show', $term->id) }}"
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.delete-btn')) {
                e.preventDefault();
                if (confirm('آیا مطمئن هستید که می‌خواهید این ترم را حذف کنید؟')) {
                    const termId = e.target.closest('.delete-btn').dataset.id;
                    const url = "{{ route('dashboard.terms.destroy', ':id') }}".replace(':id', termId);
