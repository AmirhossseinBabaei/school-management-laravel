...
@section('title', 'ویرایش درس | سیستم مدیریت مدرسه')
            ...
            <div class="hero glass-effect p-4 mb-4 animate__animated animate__fadeInUp" style="background: linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%);">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <div>
                        ...
                        <p class="text-muted mb-0">ویرایش اطلاعات درس: <span class="fw-semibold text-primary">{{ $data['lesson']->name }}</span></p>
                    </div>
            ...
            <div class="card glass-effect border-0 shadow-lg animate__animated animate__fadeInUp" style="animation-delay: 0.2s;">
                <div class="card-header glass-effect border-0">
                    <h5 class="mb-0 gradient-text fw-bold">
                        <i class="fa-solid fa-edit me-2"></i>فرم ویرایش درس
                    </h5>
                </div>
                <div class="card-body p-4">
                    <form method="post" action="{{ route('dashboard.lessons.update', $data['lesson']->id) }}">
                        @csrf
                        @method('put')
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" name="name" class="form-control" value="{{ $data['lesson']->name }}" placeholder="مثال: ریاضی، فیزیک، شیمی" value="{{ old('name') }}">
                            </div>

                            <div class="col-12">
                                <label class="form-label fw-semibold">
                                    <i class="ffa-solid fa-togfgle-on me-2 text-success"></i>رشته تحصیلی
                                </label>
                                <select name="study_field_id" class="form-control">
                                    <option value="{{ $data['lesson']->study_field->id }}">{{ $data['lesson']->study_field->name  }}</option>
                            ...
                            <div class="col-12">
                                <label class="form-label fw-semibold">
                                    <i class="fass-solid fa-toggle-on me-2 text-success"></i> پایه تحصیلی
                                </label>
                                <select name="study_base_id" class="form-control">
                                    <option value="{{ $data['lesson']->study_base->id }}">{{ $data['lesson']->study_base->name  }}</option>
